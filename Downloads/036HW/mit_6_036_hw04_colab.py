# -*- coding: utf-8 -*-
"""MIT 6.036 HW04 Colab

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/18I8zLoLsfYt0J2Bqs7LTO3lgGtassrYH
...
def gd(f, df, x0, step_size_fn, max_iter):
    ...
    for i in range(max_iter):
        current_fx = f(xs[i])
        fs.append(current_fx)
        current_step = step_size_fn(i)
        current_dfx = df(xs[i])
        current_x = xs[i]-current_step*current_dfx
        xs.append(current_x)
    x_final = xs[max_iter-1]
    return (x_final,fs,xs)
...
def num_grad(f, delta=0.001):
    def df(x):
        ...
        for i in range(x.shape[0]):
          delta_column = np.zeros((x.shape[0],1))
          delta_column[i,0]+=delta
          gradient = (f(x+delta_column)-f(x-delta_column))/(2*delta)
          df[i] = gradient
        return df
    return df
...
def hinge_loss(x, y, th, th0):
    ...
    for i in range(x.shape[1]):
      v.append(y[0,i]*(np.transpose(th)@x[:,i:i+1]+th0))
      print(v)
      sum+= hinge(v[i])
    return sum
...
def svm_obj(x, y, th, th0, lam):
    n = x.shape[1]
    temp = hinge_loss(x,y,th,th0)/n
    result = temp+lam*np.linalg.norm(th)*np.linalg.norm(th)
    passon = result
    return passon
...
def d_hinge_loss_th(x, y, th, th0):
    ...
    i = 0
    for i in range(x.shape[1]):
      v.append(y[0,i]*(np.transpose(th)@x[:,i:i+1]+th0))#v[i] is a scalor
      #if d_hinge(v[i]) == 0:
      if d_hinge(v[i]) !=0: 
        gradient[:,i:i+1] = -y[0,i]*x[:,i:i+1]
    return gradient

# Returns the gradient of hinge_loss(x, y, th, th0) with respect to th0
def d_hinge_loss_th0(x, y, th, th0):
    gradient_th0 = np.zeros((1,x.shape[1]))
    sum = 0
    v = []
    i = 0
    for i in range(x.shape[1]):
...
def d_svm_obj_th(x, y, th, th0, lam):
    n = x.shape[1]
    gradient_svm = d_hinge_loss_th(x,y,th,th0)/n
    gradient_svm+=2*lam*th
    return gradient_svm

# Returns the gradient of svm_obj(x, y, th, th0) with respect to th0
def d_svm_obj_th0(x, y, th, th0, lam):
    n = x.shape[1]
    gradient_svm_th0 = d_hinge_loss_th0(x,y,th,th0)/n
    return gradient_svm_th0
